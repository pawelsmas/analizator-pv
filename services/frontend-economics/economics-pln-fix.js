console.log('🚀 economics-pln-fix.js LOADED - VERSION PLN-FIX - timestamp:', new Date().toISOString());
...
function getEconomicParameters() {
  ...
  return {
    ...
    k1_min: systemSettings?.k1_min !== undefined ? systemSettings.k1_min : 80,
    k1_max: systemSettings?.k1_max !== undefined ? systemSettings.k1_max : 100,
    k1_coeff: systemSettings?.k1_coeff !== undefined ? systemSettings.k1_coeff : 1.3,
    k2_min: systemSettings?.k2_min !== undefined ? systemSettings.k2_min : 60,
    k2_max: systemSettings?.k2_max !== undefined ? systemSettings.k2_max : 79,
    k2_coeff: systemSettings?.k2_coeff !== undefined ? systemSettings.k2_coeff : 1.2,
    k3_min: systemSettings?.k3_min !== undefined ? systemSettings.k3_min : 40,
    k3_max: systemSettings?.k3_max !== undefined ? systemSettings.k3_max : 59,
    k3_coeff: systemSettings?.k3_coeff !== undefined ? systemSettings.k3_coeff : 1.1,
    k4_min: systemSettings?.k4_min !== undefined ? systemSettings.k4_min : 0,
    k4_max: systemSettings?.k4_max !== undefined ? systemSettings.k4_max : 39,
    k4_coeff: systemSettings?.k4_coeff !== undefined ? systemSettings.k4_coeff : 1.0,

    // Peak hours configuration
    peakHourStart: systemSettings?.peakHourStart !== undefined ? systemSettings.peakHourStart : 7,
    peakHourEnd: systemSettings?.peakHourEnd !== undefined ? systemSettings.peakHourEnd : 22,
    capacityFeeRate: systemSettings?.capacityFeeRate !== undefined ? systemSettings.capacityFeeRate : 219.4
  };
}
...
// ===== CAPACITY FEE CALCULATION WITH K1-K4 GROUPS =====
...
// Get Polish public holidays for a given year
function getPolishHolidays(year) {
  ...
  holidays.push(new Date(year, 7, 15));  // Wniebowzięcie Najświętszej Maryi Panny
  holidays.push(new Date(year, 10, 1));  // Wszystkich Świętych
  holidays.push(new Date(year, 10, 11)); // Święto Niepodległości
  holidays.push(new Date(year, 11, 25)); // Boże Narodzenie (pierwszy dzień)
  holidays.push(new Date(year, 11, 26)); // Boże Narodzenie (drugi dzień)
  holidays.push(new Date(year, 11, 24)); // Wigilia (dodatkowy dzień wolny)
...
// Check if a date is a workday (Monday-Friday, excluding holidays)
...
// Check if a specific hour is within peak hours
function isPeakHour(date, peakStart, peakEnd) {
...
// Calculate consumption profile (peak vs off-peak)
function calculateConsumptionProfile(consumptionData, params) {
  if (!consumptionData || !consumptionData.hourlyData || !consumptionData.hourlyData.values) {
    console.warn('⚠️ No hourly consumption data available for capacity fee calculation');
    return null;
  }
  ...
  const hourlyValues = consumptionData.hourlyData.values;
  const timestamps = consumptionData.hourlyData.timestamps;
  const peakStart = params.peakHourStart || 7;
  const peakEnd = params.peakHourEnd || 22;

  // If no timestamps, use year from consumptionData or default to 2024
  const startYear = consumptionData.year || 2024;
  const startDate = new Date(startYear, 0, 1); // January 1st

  for (let hour = 0; hour < hourlyValues.length; hour++) {
    // Determine the date for this hour
    let currentDate;
    if (timestamps && timestamps[hour]) {
      currentDate = new Date(timestamps[hour]);
    } else {
      // Fallback: calculate date based on hour offset from start of year
      currentDate = new Date(startDate.getTime() + hour * 3600000);
    }

    const consumption = hourlyValues[hour];

    if (isPeakHour(currentDate, peakStart, peakEnd)) {
...
// Classify into K1-K4 capacity fee group
function classifyCapacityFeeGroup(peakRatio, params) {
  const k1_min = params.k1_min !== undefined ? params.k1_min : 80;
  const k1_max = params.k1_max !== undefined ? params.k1_max : 100;
  const k1_coeff = params.k1_coeff !== undefined ? params.k1_coeff : 1.3;

  const k2_min = params.k2_min !== undefined ? params.k2_min : 60;
  const k2_max = params.k2_max !== undefined ? params.k2_max : 79;
  const k2_coeff = params.k2_coeff !== undefined ? params.k2_coeff : 1.2;

  const k3_min = params.k3_min !== undefined ? params.k3_min : 40;
  const k3_max = params.k3_max !== undefined ? params.k3_max : 59;
  const k3_coeff = params.k3_coeff !== undefined ? params.k3_coeff : 1.1;

  const k4_min = params.k4_min !== undefined ? params.k4_min : 0;
  const k4_max = params.k4_max !== undefined ? params.k4_max : 39;
  const k4_coeff = params.k4_coeff !== undefined ? params.k4_coeff : 1.0;

  if (peakRatio >= k1_min && peakRatio <= k1_max) {
    return { group: 'K1', coefficient: k1_coeff, peakRatio };
  }
  if (peakRatio >= k2_min && peakRatio <= k2_max) {
    return { group: 'K2', coefficient: k2_coeff, peakRatio };
  }
  if (peakRatio >= k3_min && peakRatio <= k3_max) {
    return { group: 'K3', coefficient: k3_coeff, peakRatio };
  }
  return { group: 'K4', coefficient: k4_coeff, peakRatio };
}

// Calculate capacity fee with K1-K4 classification
function calculateCapacityFeeForConsumption(consumptionData, params) {
  // If no consumption data or in simple mode, return flat fee
  if (!consumptionData || !consumptionData.hourlyData) {
    console.log('📊 Capacity fee: Using simple flat rate (no hourly data)');
    return {
      effectiveRatePerMWh: params.capacity_fee,
      totalFeePLN: null,
      kGroup: null,
      peakRatio: null,
      breakdown: null
  ...
  const profile = calculateConsumptionProfile(consumptionData, params);
  if (!profile) {
    console.log('📊 Capacity fee: Using simple flat rate (profile calculation failed)');
    return {
      effectiveRatePerMWh: params.capacity_fee,
      totalFeePLN: null,
      kGroup: null,
  ...
  // Classify into K1-K4 group
  const kGroup = classifyCapacityFeeGroup(profile.peakRatio, params);

  // Calculate capacity fee: A × Energy_peak × Rate
  const peakEnergyMWh = profile.peakConsumption / 1000; // kWh → MWh
  const capacityFeeRate = params.capacityFeeRate || params.capacity_fee || 219.4;
  const capacityFeePLN = kGroup.coefficient * peakEnergyMWh * capacityFeeRate;

  // Calculate effective rate per MWh for total consumption
  const totalEnergyMWh = profile.totalConsumption / 1000;
  const effectiveRatePerMWh = totalEnergyMWh > 0 ? capacityFeePLN / totalEnergyMWh : 0;

  console.log(`📊 Capacity fee calculation:
    - Peak consumption: ${peakEnergyMWh.toFixed(2)} MWh
    - Off-peak consumption: ${(profile.offPeakConsumption / 1000).toFixed(2)} MWh
    - Total consumption: ${totalEnergyMWh.toFixed(2)} MWh
    - Peak ratio: ${profile.peakRatio.toFixed(2)}%
    - K-group: ${kGroup.group} (coefficient: ${kGroup.coefficient})
    - Base rate: ${capacityFeeRate} PLN/MWh
    - Total capacity fee: ${capacityFeePLN.toFixed(2)} PLN
    - Effective rate: ${effectiveRatePerMWh.toFixed(2)} PLN/MWh`);

  // Return full details for breakdown visualization
  return {
    effectiveRatePerMWh: effectiveRatePerMWh,
    totalFeePLN: capacityFeePLN,
    kGroup: kGroup.group,
    kCoefficient: kGroup.coefficient,
    peakRatio: profile.peakRatio,
    breakdown: {
      peakConsumptionMWh: peakEnergyMWh,
      offPeakConsumptionMWh: profile.offPeakConsumption / 1000,
      totalConsumptionMWh: totalEnergyMWh,
      baseRate: capacityFeeRate
    }
  };
}
...
async function performEconomicAnalysis() {
  ...
  try {
    ...
    const capacityFeeResult = calculateCapacityFeeForConsumption(consumptionData, params);
    const totalEnergyPriceWithCapacity = totalEnergyPrice + capacityFeeResult.effectiveRatePerMWh;
...
function generateSensitivityAnalysisCharts() {
  ...
  const totalEnergyPrice = calculateTotalEnergyPrice(params);
  const capacityFeeResult = calculateCapacityFeeForConsumption(consumptionData, params);
  const totalPriceWithCapacity = totalEnergyPrice + capacityFeeResult.effectiveRatePerMWh;
...
function exportEaaSToExcel() {
  ...
  // Calculate capacity fee with K1-K4 details
  const capacityFeeResult = calculateCapacityFeeForConsumption(consumptionData, params);
  ...
  const tariffComponents = {
    ...
    oze: params.oze_fee,
    cogeneration: params.cogeneration_fee,
    capacity: capacityFeeResult.effectiveRatePerMWh,
    capacityDetails: capacityFeeResult, // Add full details for breakdown
    excise: params.excise_tax
  };
  ...
  const summaryData = [
    ...
    ['Opłata mocowa (efektywna):', tariffComponents.capacity],
    [''],
    ['ROZBICIE OPŁATY MOCOWEJ'],
    ...(tariffComponents.capacityDetails.kGroup ? [
      ['Grupa K:', tariffComponents.capacityDetails.kGroup],
      ['Współczynnik A:', tariffComponents.capacityDetails.kCoefficient],
      ['Udział szczyt [%]:', tariffComponents.capacityDetails.peakRatio.toFixed(2)],
      ['Energia szczyt [MWh]:', tariffComponents.capacityDetails.breakdown.peakConsumptionMWh.toFixed(2)],
      ['Energia poza szczytem [MWh]:', tariffComponents.capacityDetails.breakdown.offPeakConsumptionMWh.toFixed(2)],
      ['Stawka bazowa [PLN/MWh]:', tariffComponents.capacityDetails.breakdown.baseRate],
      ['Całkowita opłata mocowa [PLN/rok]:', tariffComponents.capacityDetails.totalFeePLN.toFixed(2)]
    ] : [
      ['Tryb prosty (bez K1-K4):', 'Tak']
    ]),
    [''],
