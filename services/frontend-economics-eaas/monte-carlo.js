...
async function runMonteCarlo() {
    ...
    const capexUncertainty = parseFloat(document.getElementById('mcCapexUncertainty')?.value || 8);
    const inflationUncertainty = parseFloat(document.getElementById('mcInflationUncertainty')?.value || 1.5);
    ...
    try {
        ...
        console.log('🎲 MC: Uncertainties - price:', priceUncertainty, '%, production:', productionUncertainty, '%, capex:', capexUncertainty, '%, inflation:', inflationUncertainty, 'pp');

        // Build request with user-defined uncertainties
        // Sources: NREL P50/P90, SolarGIS, FfE 2024, IMF volatility study
        const request = {
            n_simulations: nSimulations,
            electricity_price_uncertainty_pct: priceUncertainty,  // Default: 12% (bankable), max: 30%
            production_uncertainty_pct: productionUncertainty,    // Default: 8% (NREL standard), max: 20%
            capex_uncertainty_pct: capexUncertainty,              // Default: 8% (post-EPC bid), max: 20%
            inflation_uncertainty_pct: inflationUncertainty,      // Default: 1.5pp (NBP), max: 15pp
        ...
        lastMcSimulationData = {
            ...
            settings: {
                ...
                capexUncertainty: capexUncertainty,
                inflationUncertainty: inflationUncertainty
            }
        };
