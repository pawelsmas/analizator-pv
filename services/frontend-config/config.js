...
async function runAnalysis() {
  ...
  try {
    ...
    const pvConfig = {
      ...
      // DC/AC selection mode ('manual' or 'auto')
      dcac_mode: systemSettings?.dcacMode || 'manual',
      // Weather data source (PVGIS or clearsky)
      use_pvgis: systemSettings?.weatherDataSource === 'pvgis',
      // Environmental parameters (Advanced)
      altitude: systemSettings?.altitude || 100,
      albedo: systemSettings?.albedo || 0.2,
      soiling_loss: (systemSettings?.soilingLoss || 2) / 100  // Convert % to decimal
    };
...
function selectMasterVariant() {
  ...
  if (infoDiv) {
    const dcacInfo = variant.dcac_ratio ? `, DC/AC: ${variant.dcac_ratio.toFixed(2)}` : '';
    infoDiv.innerHTML = `
      ✓ Wybrany wariant: <strong>${selectedVariant}</strong>
      (${(variant.capacity/1000).toFixed(2)} MWp${dcacInfo},
